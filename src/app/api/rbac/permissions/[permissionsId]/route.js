...
export async function DELETE(req, context) {
    try {
        ...
        if (!mongoose.Types.ObjectId.isValid(context.params.permissionsId)) return NextResponse.json({ success: false, message: "Permission not exists" }, { status: 400 })

        const permissions = await Permissions.findByIdAndDelete({ _id: context.params.permissionsId })

        if (!permissions) return NextResponse.json({ success: false, message: "Permission not exists" }, { status: 400 })
